...
//		InterfaceAPISingleton.getInstance().setbClass(loadedClass);
	...
	public static ArrayList<Tree> dependencyGenerator(Path path) {
		...
		int index = 0;
		int nodeId = 0;
		ArrayList<PNode> nodeList = path.getListofNodes();
		...
		for(PNode node: nodeList){
				...
				if (node.getIns().getOpcodeCommand() == "iadd") {
					...
					PNode rootNode = new NonPredicateNode(nodeId++, node.getIns());
					PNode leftLeaf = new NonPredicateNode(nodeId++, node.getIns());
					PNode rightLeaf = new NonPredicateNode(nodeId++, node.getIns());
	...
	// else if instruction at index = 2 is other than iadd then, take what ever instruction available 
	// at index = 0  
	public static PNode getSecondParameter(ArrayList<PNode> nodeList,
			int index, int tempVarCounter, HashMap<Integer, String> dependencyDataCollector) {
		PNode rightLeaf = new NonPredicateNode();
